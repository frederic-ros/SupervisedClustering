# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 11:46:41 2024
...
def transformer_labels(dict_labels):
    # Extraire les labels de la clé 'train'
    labels = dict_labels['train']
    
    # Vérifier le type de labels
    #print("Type des labels : ", type(labels))
    
    if isinstance(labels, np.ndarray):  # Si c'est un tableau NumPy, convertis-le en liste
        labels = labels.tolist()
    
    # Créer un dictionnaire pour mapper les labels
    unique_labels = np.unique(labels)
    label_mapping = {}
    
    # Initialiser le compteur pour les labels autres que -1 et -2
    current_label = 0
    for label in unique_labels:
        if label != -1 and label != -2:
            ...
            label_mapping[label] = label  # Garder -1 et -2 inchangés
    
    # Afficher le mappage des labels pour validation
   # print("Label Mapping : ", label_mapping)
    
    # Appliquer la transformation sur les labels
    transformed_labels = np.array([label_mapping[label] for label in labels])
    
    # Vérifier si les labels ont été correctement transformés
    #print("Labels transformés : ", transformed_labels)
    
    return transformed_labels

def convert_dict_to_labels(cluster_dict, n_points):
    labels = np.full(n_points, -1, dtype=int)  # -1 par défaut = bruit
    for class_id, indices in cluster_dict.items():
        # Gérer le bruit
        if isinstance(class_id, str) and class_id.lower() == 'noise':
            for idx in indices:
                if 0 <= idx < n_points:
                    labels[idx] = -1
            continue

        # Gérer les vrais clusters numérotés
        try:
            class_id_int = int(class_id)
        except (ValueError, TypeError):
            continue  # on saute si ce n'est pas un entier (ex : 'train')

        for idx in indices:
            try:
                idx = int(idx)
                if 0 <= idx < n_points:
                    labels[idx] = class_id_int
            except:
                continue
    return labels
...
class DenMune():
    ...
    def kd_NGT(self):
        if len(self.dp_dis) == 0:
            ...
            # Requête des k+1 plus proches voisins (le premier est le point lui-même)
            dists, indices = tree.query(self.data, k=k + 1)

            self.dp_dis = []
            for i in range(self.dp_count):
                neighbors = []
                for idx, dist in zip(indices[i][1:], dists[i][1:]):  # on saute soi-même (position 0)
                    neighbors.append((idx, round(dist, 6)))
                self.dp_dis.append(neighbors)
