...
def sdbscan(X, min_samples=20, min_cluster_size=None, noise_percent=0.0, metric='euclidean',
    ...
    neighbors = [np.extract(nearest_list[0][i] <= radius[i], nearest_list[1][i]) 
                 for i in range(radius.shape[0])]
    
    
    #neighbors = np.stack(neighbors, axis=0 )
    #conditions = [nearest_list[0][i] <= radius[i] for i in range(radius.shape[0])]

    '''
    neighbors = []
    for i in range(radius.shape[0]):
        if nearest_list[0][i] <= radius[i]:
            neighbors[i] = nearest_list[1][i]
    '''
    ...
    lables = _sdbscan_merge_chain(is_core, 
                                  chain_index, 
                                  neighbors, 
                                  nearest_list[0], 
                                  lables, 
