# -*- coding: utf-8 -*-
"""
Created on Wed Nov 26 08:01:31 2025
...
Auteur: Version professionnelle avec argparse et main
...
# -----------------------------
# 5. Extraction embeddings/probs
# -----------------------------
def extract_embeddings_probs(loader, backbone, head, device):
    backbone.eval()
    head.eval()
...
def mainscanminst(args):
    ...
    if args.dataset == 'MNIST':
        dataset_full = datasets.MNIST(str(default_data_dir), train=True, download=True, transform=transforms.ToTensor())
        test_dataset = datasets.MNIST(str(default_data_dir), train=False, download=True, transform=transforms.ToTensor())
    elif args.dataset == 'KMNIST':
        dataset_full = datasets.KMNIST(str(default_data_dir), train=True, download=True, transform=transforms.ToTensor())
        test_dataset = datasets.KMNIST(str(default_data_dir), train=False, download=True, transform=transforms.ToTensor())
    else:
        raise ValueError("Dataset inconnu")

    train_dataset = balanced_subset(dataset_full, args.n_samples_per_class)
    ...
    optimizer = optim.Adam(list(backbone.parameters()) + list(head.parameters()), lr=args.learning_rate)
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs} - Loss: {total_loss/len(train_dataset):.4f}")

    # Extraction embeddings
    train_embeddings, train_probs, train_labels = extract_embeddings_probs(train_loader, backbone, head, device)
    test_embeddings, test_probs, test_labels = extract_embeddings_probs(test_loader, backbone, head, device)
...
# -----------------------------
def launchscanmnist():
    parser = argparse.ArgumentParser(description="SCAN clustering on MNIST/KMNIST")
    parser.add_argument("--dataset", type=str, default="MNIST", help="Dataset to use: MNIST or KMNIST")
    ...
    parser.add_argument("--device", type=str, default=None, help="Specify cuda or cpu or leave None for auto")
